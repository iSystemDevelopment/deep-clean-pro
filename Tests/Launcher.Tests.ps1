...
Describe 'gist-launcher.ps1 - Admin + argument handling' -Tag 'Unit','Security' {
    ...
    It 'Should build arguments from environment variables: DCP_PROFILE, DCP_QUICK_MODE, DCP_NO_REBOOT, DCP_RUN_UPDATES, DCP_SKIP_EXTENSIONS, DCP_HARDEN, DCP_HARDEN_STRICT' {
        $content | Should -Match '\$env:DCP_PROFILE'
        $content | Should -Match '\$env:DCP_QUICK_MODE'
        $content | Should -Match '\$env:DCP_NO_REBOOT'
        $content | Should -Match '\$env:DCP_RUN_UPDATES'
        $content | Should -Match '\$env:DCP_SKIP_EXTENSIONS'
        $content | Should -Match '\$env:DCP_HARDEN'
        $content | Should -Match '\$env:DCP_HARDEN_STRICT'
    }
