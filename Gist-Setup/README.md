...
| Variable | Effect |
|----------|--------|
| `DCP_PROFILE` | `-Profile` (Balanced, Gaming, Development, Music, Video, Office) |
| `DCP_QUICK_MODE` | `-QuickMode` when `true` |
| `DCP_NO_REBOOT` | `-NoReboot` when `true` |
| `DCP_RUN_UPDATES` | `-RunWindowsUpdates` when `true` |
| `DCP_SKIP_EXTENSIONS` | `-SkipExtensions` when `true` |
| `DCP_HARDEN` | `-Harden` when `true` |
| `DCP_HARDEN_STRICT` | `-HardenStrict` when `true` |
| `DCP_NO_PAUSE` | Skip Enter-to-exit on launcher |

Matches **Deep Clean Pro 2.5.0+** engine env handling on `main`.
...
(Or download the engine to a file and run `& $path -WhatIf`.)
