...
Checks pending Windows Updates via Microsoft Update COM API. Installs when `-RunWindowsUpdates` is set (or user confirms in an interactive console).

### **Signature**

```powershell
# Called automatically from main; install gate:
.\DeepCleanPro.ps1 -RunWindowsUpdates
# Env: DCP_RUN_UPDATES=true
```

### **Behavior**

* Always **searches** and reports pending Software updates (titles)
* Installs only if `-RunWindowsUpdates` / `DCP_RUN_UPDATES=true`, or interactive `Y` at prompt
* Honors `-WhatIf` (search still runs; download/install is skipped)
* Skips updates that require user input
* Reports reboot-required from installer
* Health check (`Test-SystemHealth`) also reports update count separately
