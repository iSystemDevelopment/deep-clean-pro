...
Local BATs / Control Center call **`Scripts/Invoke-DcpElevated.ps1`**, which prefers `C:\DeepCleanPro\DeepCleanPro.ps1` or downloads to a temp file, then `Start-Process -Verb RunAs -File` (no broken `irm | iex; DeepCleanPro.ps1 ...`).

Online / Gist path uses **`Gist-Setup/gist-launcher.ps1`**:
