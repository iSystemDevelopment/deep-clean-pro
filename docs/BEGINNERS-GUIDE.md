...
# 🖥️ 2. Control Center / Shortcuts (Recommended for Beginners)

Don’t want to paste commands? Use the in-repo BAT pack (or create Desktop links).
...
2. Double-click **`Control-center.bat`**, *or* any file under **`Shortcuts\`**
3. Optional — create Desktop `.lnk` entries:

```powershell
.\Scripts\CreateDesktopShortcuts.ps1
```

4. Approve the Administrator (UAC) prompt when asked

Each shortcut automatically:

* Elevates via `Scripts\Invoke-DcpElevated.ps1`
* Loads the correct profile / switches
* Applies safe defaults (`-NoReboot` on most BATs)
